// build
// nvcc image_blur.cu -o image_blur -ljpeg

// execute
// image_blur <img_path.jpg>

// note: works for b&w image at the moment, striding in channels is necessary to support arbitrary image bluring.
...
__global__ void blur_kernel(unsigned char *im_out, unsigned char *im_in, int width, int height, int kernel_size)
{
    ...
    if (col_idx < width && row_idx < height)
    {
        ...
        for (int i = -kernel_size; i < kernel_size + 1; i++)
        {
            ...
            {
                int cur_row = row_idx + i;
                int cur_col = col_idx + j;

                if (cur_row < 0 || cur_col < 0 || cur_row == height || cur_col == width)
                    continue;

                pix_val += im_in[cur_row * width + cur_col];
                n_pixels += 1;
            }
        }
        im_out[row_idx * width + col_idx] = (unsigned char)(pix_val / n_pixels);
...
int main(int argc, char *argv[])
{
    ...
    blur_kernel<<<dimGrid, dimBlock>>>(im_out_d, im_in_d, width, height, 10);
    ...
    save_image("out_blur.jpg", gray_buffer, width, height, JCS_GRAYSCALE, 1);
